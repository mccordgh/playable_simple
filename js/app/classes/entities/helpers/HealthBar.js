define(['Helper', 'Assets'], function(Helper, Assets){
	...
	var HealthBar = Helper.extend({
		init: function(_handler, _entity, _properties){
			...
			this.setProperties(_properties, _entity);
			// setInterval(() => { _entity.takeDamage(20);}, 2000);
		},
		setProperties: function(_properties, _entity) {
			this.entity = _entity;
		...
		render(_g){
				if (this.nodes <= 0 || (this.entity.states && this.entity.state === this.entity.states.dying)) {
					return;
				}
				// if (this.fixed){
					...
					// if (this.nodes < this.totalNodes || ) {
						...
						if (this.renderOnFull === "on" || this.entity.health < this.start){
							if (this.border.show){
								...
								_g.fillRect(this.entity.getX() - this.xOffset - this.border.width,
														this.entity.getY() - this.yOffset - this.border.width,
							...
							for (let i = 0; i < this.totalNodes; i++){
								...
								_g.fillRect(-(this.nodeWidth * this.totalNodes) + (this.split / 2) + (this.totalNodes * this.nodeWidth) + (this.nodeWidth * i) + this.entity.getX() - this.xOffset,
														this.entity.getY() - this.yOffset,
